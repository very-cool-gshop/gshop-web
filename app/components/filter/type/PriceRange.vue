...
const filterToComponent = (filter: ProductFilter[]) =>
    (filter
        ...
        .at(0) as FilterValue) ?? input.value
...
<template>
    <UForm :state="state" :schema="schema" class="flex flex-col gap-4">
        <div class="flex flex-row gap-4">
            <UFormField name="min" label="from">
                <UInputNumber v-model="state.min" class="w-24" :min="0" :max="state.max" @change="submit(state)" />
            </UFormField>

            <UFormField name="max" label="to">
                <UInputNumber v-model="state.max" class="w-24" :min="state.min" @change="submit(state)" />
            </UFormField>
        </div>
    </UForm>
</template>
